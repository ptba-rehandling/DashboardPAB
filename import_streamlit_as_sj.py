...
# Filter tanggal untuk target rakor (berdasarkan range waktu yang dipilih)
...
# Validasi rentang tanggal target rakor
if rakor_start > rakor_end:
    st.error("Tanggal awal target rakor tidak boleh lebih besar dari tanggal akhir.")
    st.stop()

# Menghitung jumlah hari dalam rentang target rakor
...
# Menampilkan input manual untuk target rakor per hari
st.sidebar.header("Input Target Harian Rakor:")
rakor_daily_targets = {}
for key, value in rakor_targets.items():
    rakor_daily_targets[key] = st.sidebar.number_input(f"Target harian untuk {key}", value=value/num_days_rakor)

# Menampilkan target harian rakor di sidebar
...
for spph_mitra in df_filtered['spph'].unique():
    spph_mitra_targets[spph_mitra] = st.sidebar.number_input(f"Target for {spph_mitra}", value=1000)
...
st.write(spph_mitra_df.style.background_gradient(cmap="Greens"))
